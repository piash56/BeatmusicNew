...
@section('page-title', 'Stream Analytics')
@section('page-subtitle', 'Track your music performance')
...
<div class="space-y-6" x-data="{ search: @json(request('search', '')), submitSearch(){ this.$refs.searchForm.submit() } }">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-gray-900 rounded-2xl border border-white/5 p-5">
            <div class="flex items-center justify-between mb-3">
                ...
                <span class="text-xs text-green-400 bg-green-900/30 px-2 py-1 rounded-full">Total</span>
            </div>
        ...
        <div class="bg-gray-900 rounded-2xl border border-white/5 p-5">
            <div class="flex items-center justify-between mb-3">
                <span class="text-2xl">🚀</span>
                <span class="text-xs text-purple-400 bg-purple-900/30 px-2 py-1 rounded-full">Released</span>
            </div>
            <div class="text-2xl font-bold text-white">{{ number_format($summary['released_count'] ?? 0) }}</div>
            <div class="text-xs text-gray-400 mt-1">Released Tracks & Albums</div>
        </div>
        <div class="bg-gray-900 rounded-2xl border border-white/5 p-5">
            <div class="flex items-center justify-between mb-3">
                <span class="text-2xl">💾</span>
            ...
            <div class="text-xs text-gray-400 mt-1">Total Pre-saves</div>
        </div>
    </div>
    ...
    <div class="bg-gray-900 rounded-2xl border border-white/5 overflow-hidden">
        <div class="p-4 border-b border-white/5 flex items-center justify-between">
            <h3 class="font-semibold text-white">Releases Performance</h3>
        ...
        <div class="p-12 text-center">
            ...
            <p class="text-gray-400 text-sm">No released tracks yet.</p>
            <a href="{{ route('dashboard.releases.create') }}?new=1" class="text-purple-400 hover:text-purple-300 text-sm mt-2 inline-block">Upload your first track →</a>
        </div>
        @else
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-800/50 border-b border-white/5">
                    <tr>
                        <th class="text-left px-4 py-3 text-gray-400 font-medium">Release</th>
                        <th class="text-right px-4 py-3 text-gray-400 font-medium">Total Streams</th>
                        <th class="text-right px-4 py-3 text-gray-400 font-medium hidden sm:table-cell">New Streams</th>
                        <th class="text-right px-4 py-3 text-gray-400 font-medium hidden md:table-cell">Pre-saves</th>
                        <th class="text-right px-4 py-3 text-gray-400 font-medium hidden lg:table-cell">Revenue</th>
                        <th class="text-left px-4 py-3 text-gray-400 font-medium hidden lg:table-cell">Status</th>
                    </tr>
                </thead>
