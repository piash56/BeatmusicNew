...
@section('title', 'Modifica Rilascio')
@section('page-title', 'Modifica Rilascio')
...
<div x-data="{
    ...
    <a href="{{ route('dashboard.releases.show', $track->id) }}" class="inline-flex items-center space-x-2 text-gray-400 hover:text-white transition text-sm mb-6">
        ...
        <span>Torniamo al rilascio</span>
    </a>
    ...
    <div class="mb-6 p-4 rounded-xl bg-amber-900/30 border border-amber-500/30 text-amber-200 text-sm flex items-center space-x-2">
        ...
        <span>La modifica di questa versione la sposterà nella sezione "Modifica in sospeso" per la revisione.</span>
    </div>
    @endif
    ...
    <form method="POST" action="{{ route('dashboard.releases.update', $track->id) }}" enctype="multipart/form-data" id="edit-release-form" data-upload-cover-url="{{ route('dashboard.releases.upload-cover') }}" data-upload-audio-url="{{ route('dashboard.releases.upload-audio') }}" @submit="onEditSubmit($event)">
        ...
        <div x-show="step === 1" x-cloak class="glass rounded-2xl p-8">
            <h2 class="text-xl font-bold text-white mb-6 text-center">Tipo di rilascio</h2>
            <div class="max-w-md mx-auto text-center">
                <p class="text-gray-300 text-lg capitalize">{{ $track->release_type }}</p>
                <p class="text-gray-500 text-sm mt-2">Il tipo di rilascio non può essere modificato durante la modifica.</p>
            </div>
            <div class="flex justify-end mt-8">
                <button type="button" @click="step = 2" class="px-8 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-semibold rounded-xl transition">Prossimo →</button>
            </div>
        </div>
        ...
        <div x-show="step === 2" x-cloak class="glass rounded-2xl p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-1.5">Titolo Album</label>
            ...
            <div class="flex justify-between mt-8">
                ...
                <button type="button" @click="step = 3" class="px-8 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-semibold rounded-xl transition">Prossimo →</button>
            </div>
        </div>
        ...
        <div x-show="step === 3" x-cloak class="glass rounded-2xl p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Nome di battesimo</label><input type="text" name="first_name" value="{{ old('first_name', $track->first_name) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Cognome</label><input type="text" name="last_name" value="{{ old('last_name', $track->last_name) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Nome d'arte</label><input type="text" name="stage_name" value="{{ old('stage_name', $track->stage_name) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Artista principale <span class="text-red-400">*</span></label><input type="text" name="artists" value="{{ old('artists', $track->artists) }}" required class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-300 mb-1.5">Dotato</label><input type="text" name="featuring_artists" value="{{ old('featuring_artists', $track->featuring_artists) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Data di rilascio <span class="text-red-400">*</span></label><input type="date" name="release_date" value="{{ old('release_date', $track->release_date ? $track->release_date->format('Y-m-d') : '') }}" required class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                ...
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Autori</label><input type="text" name="authors" value="{{ old('authors', $track->authors) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Compositori</label><input type="text" name="composers" value="{{ old('composers', $track->composers) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Produttore</label><input type="text" name="producer" value="{{ old('producer', $track->producer) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Genere primario <span class="text-red-400">*</span></label>
                ...
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Genere secondario</label><input type="text" name="secondary_genre" value="{{ old('secondary_genre', $track->secondary_genre) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div class="md:col-span-2"><label class="flex items-center space-x-2 cursor-pointer"><input type="checkbox" name="is_explicit" value="1" {{ old('is_explicit', $track->is_explicit) ? 'checked' : '' }} class="w-4 h-4 rounded border-white/20 bg-white/5 text-purple-600"><span class="text-sm text-gray-300">Contenuto esplicito</span></label></div>
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Ora di inizio di TikTok</label><input type="text" name="tik_tok_start_time" value="{{ old('tik_tok_start_time', $track->tik_tok_start_time) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500"></div>
                <div><label class="block text-sm font-medium text-gray-300 mb-1.5">Durata della canzone</label><input type="text" name="song_duration" value="{{ old('song_duration', $track->song_duration) }}" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500" placeholder="00:00:00"></div>
                <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-300 mb-1.5">Breve biografia</label><textarea name="short_bio" rows="3" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500 resize-none">{{ old('short_bio', $track->short_bio) }}</textarea></div>
                <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-300 mb-1.5">Descrizione della traccia</label><textarea name="track_description" rows="3" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500 resize-none">{{ old('track_description', $track->track_description) }}</textarea></div>
                <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-300 mb-1.5">Società di gestione collettiva (facoltativo)</label>
                    <div class="space-y-2">
                ...
                <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-300 mb-1.5">Dettagli sulla distribuzione</label><textarea name="distribution_details" rows="2" class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500 resize-none">{{ old('distribution_details', $track->distribution_details) }}</textarea></div>
                <div class="md:col-span-2"><label class="flex items-center space-x-2 cursor-pointer"><input type="checkbox" name="is_youtube_beat" value="1" x-model="isYoutubeBeat" {{ old('is_youtube_beat', $track->is_youtube_beat) ? 'checked' : '' }} class="w-4 h-4 rounded border-white/20 text-purple-600"><span class="text-sm text-gray-300">Batti da YouTube</span></label></div>
                <div class="md:col-span-2" x-show="isYoutubeBeat" x-cloak><label class="flex items-center space-x-2 cursor-pointer"><input type="checkbox" name="has_license" value="1" {{ old('has_license', $track->has_license) ? 'checked' : '' }} class="w-4 h-4 rounded border-white/20 text-purple-600"><span class="text-sm text-gray-300">Ho la licenza</span></label></div>
            </div>
            <div class="flex justify-between mt-8">
                ...
                <button type="button" @click="step = 4" class="px-8 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-semibold rounded-xl transition">Prossimo →</button>
            </div>
        </div>
        ...
        <div x-show="step === 4" x-cloak class="glass rounded-2xl p-6">
            ...
            <div class="flex justify-between mt-8">
                ...
                <button type="button" @click="step = 5" class="px-8 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-semibold rounded-xl transition">Prossimo →</button>
            </div>
        </div>
        ...
        <div x-show="step === 5 && hasSpotifyApple === 'YES'" x-cloak class="glass rounded-2xl p-6">
            ...
            <div class="flex justify-between mt-8">
                ...
                <button type="button" @click="step = 6" class="px-8 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-semibold rounded-xl transition">Prossimo →</button>
            </div>
        </div>
        ...
        <div x-show="(step === 5 && hasSpotifyApple === 'NO') || (step === 6 && hasSpotifyApple === 'YES')" x-cloak class="glass rounded-2xl p-6">
            ...
            <div class="flex justify-between mt-8">
                <button type="button" @click="step = (hasSpotifyApple === 'YES' ? 5 : 4)" class="px-6 py-2.5 bg-white/10 hover:bg-white/15 text-white font-medium rounded-xl transition text-sm">← Indietro</button>
                <button type="button" @click="step = (hasSpotifyApple === 'YES' ? 7 : 6)" class="px-8 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-semibold rounded-xl transition">Prossimo →</button>
            </div>
        </div>
        ...
        <div x-show="(step === 6 && hasSpotifyApple === 'NO') || (step === 7 && hasSpotifyApple === 'YES')" x-cloak class="glass rounded-2xl p-6 space-y-8">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-1.5">Copertina</label>
                @if($track->cover_art)
                <div class="flex items-center gap-4 mb-3" x-show="!coverPreview && !coverFilePath">
                    ...
                    <span class="text-gray-400 text-sm">Copertina attuale. Carica un nuovo file per sostituirlo.</span>
                </div>
                @endif
                <div x-show="coverPreview" x-cloak class="flex items-center space-x-4 mb-3">
                    ...
                    <span class="text-gray-400 text-sm">Nuova copertina selezionata. Clicca su × per rimuoverla.</span>
                </div>
            ...
            <div>
                ...
                <label class="block text-sm font-medium text-gray-300 mb-1.5">File audio</label>
                @if($track->audio_file)
                <div class="mb-3 p-3 rounded-xl bg-white/5 border border-white/10" x-show="!audioFilePath">
                    <p class="text-gray-400 text-sm mb-2">Traccia corrente. Carica un nuovo file da sostituire.</p>
                    <audio controls class="w-full max-w-md h-10" src="{{ route('files.audio', $track->id) }}">Il tuo browser non supporta l'elemento audio.</audio>
                ...
                <div x-show="audioFilePath" x-cloak class="mb-3 flex items-center gap-2">
                    <span class="text-green-400 text-sm">Traccia caricata</span>
            ...
            <div>
                ...
                <textarea name="lyrics" rows="6" required class="w-full bg-white/5 border border-white/10 text-white px-4 py-3 rounded-xl focus:outline-none focus:border-purple-500 resize-none" placeholder="Incolla o digita il testo...">{{ old('lyrics', $track->lyrics) }}</textarea>
            </div>
