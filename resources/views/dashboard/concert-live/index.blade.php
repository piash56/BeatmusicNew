...
@section('title', 'Concerto dal vivo')
@section('page-title', 'Concerto dal vivo')
@section('page-subtitle', 'Richiedi slot per spettacoli dal vivo')
...
<div class="space-y-6">
    ...
    <div class="bg-gray-900 rounded-2xl border border-white/5 p-6" x-data="{ open: false }">
        <div class="flex items-center justify-between">
            <div>
                <h3 class="font-semibold text-white">Richiedi uno slot live</h3>
                <p class="text-gray-400 text-sm mt-0.5">Candidati per un'opportunità di esibizione dal vivo in un concerto</p>
            </div>
            ...
            <form method="POST" action="{{ route('dashboard.concert-live.request') }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        <label class="block text-sm text-gray-400 mb-1.5">Seleziona lo spazio per il concerto <span class="text-red-400">*</span></label>
                        <select name="concert_live_id" required class="w-full bg-gray-800 border border-white/10 text-gray-300 px-4 py-2.5 rounded-xl focus:outline-none focus:border-purple-500 text-sm">
                            <option value="">Seleziona lo spazio per il concerto...</option>
                            @foreach($concerts as $slot)
                            <option value="{{ $slot->id }}">
                                {{ $slot->name }} — {{ $slot->concert_date ? $slot->concert_date->format('M d, Y') : '—' }} ({{ $slot->city }}) — {{ $slot->slots_remaining }} slot rimasti
                            </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm text-gray-400 mb-1.5">Nome dell'artista <span class="text-red-400">*</span></label>
                        <input type="text" name="artist_name" required maxlength="100" placeholder="Il tuo nome da artista"
                    ...
                    <div class="sm:col-span-2">
                        <p class="text-xs text-gray-500">Dopo l'invio, un amministratore esaminerà e confermerà/annullerà la tua richiesta.</p>
                    </div>
                </div>
                <div class="flex justify-end space-x-3 mt-4">
                    ...
                    <button type="submit" class="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm rounded-xl transition">Invia richiesta</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="bg-gray-900 rounded-2xl border border-white/5 p-6">
        <h3 class="font-semibold text-white mb-4">Slot per concerti disponibili</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-gradient-to-br from-purple-900/20 to-indigo-900/20 border border-purple-500/20 rounded-xl p-4">
                ...
                @endif
                <div class="mt-3 text-xs text-gray-500">Posti rimanenti: <span class="text-gray-300">{{ $slot->slots_remaining }}</span></div>
                ...
                <div class="text-xs text-gray-500 mt-1">{{ $slot->booking_percentage }}% prenotato</div>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="bg-gray-900 rounded-2xl border border-white/5 overflow-hidden">
        <div class="p-4 border-b border-white/5">
            <h3 class="font-semibold text-white">Le mie richieste dal vivo</h3>
        </div>
        ...
        <div class="p-12 text-center">
            ...
            <p class="text-gray-400 text-sm">Nessuna richiesta attiva ancora.</p>
        </div>
        @else
