...
@section('page-title', 'New Knowledge Base Article')
...
<div class="max-w-3xl space-y-6">
    ...
    <form method="POST" action="{{ route('admin.knowledge-base.store') }}" class="bg-gray-900 rounded-2xl border border-white/5 p-6 space-y-5">
        @csrf
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="category" value="{{ old('category') }}" placeholder="e.g. Distribution, Account..."
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="excerpt" rows="2" placeholder="Short summary..."
                    class="w-full bg-gray-800 border border-white/10 text-white placeholder-gray-500 px-4 py-2.5 rounded-xl focus:outline-none focus:border-purple-500 text-sm resize-none">{{ old('excerpt') }}</textarea>
            </div>
            <div class="sm:col-span-2">
                ...
                <textarea name="content" rows="16" required
                    class="w-full bg-gray-800 border border-white/10 text-white px-4 py-2.5 rounded-xl focus:outline-none focus:border-purple-500 text-sm resize-none font-mono">{{ old('content') }}</textarea>
            </div>
            <div class="sm:col-span-2">
                <label class="flex items-center space-x-2 cursor-pointer">
                    <input type="checkbox" name="status" value="active" {{ old('status') === 'active' ? 'checked' : '' }} class="w-4 h-4 rounded border-white/20 bg-gray-800 text-purple-600">
        ...
        <div class="flex justify-end space-x-3">
            ...
            <button type="submit" class="px-5 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm rounded-xl transition">Save Article</button>
        </div>
    </form>
</div>
@endsection
