...
@section('page-title', 'Aggiungi nuovo utente')
    ...
    <form method="POST"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sm:col-span-2">
                <label class="block text-sm text-gray-400 mb-1.5">Nome e cognome <span class="text-red-400">*</span></label>
            ...
            <div class="sm:col-span-2">
                <label class="block text-sm text-gray-400 mb-1.5">E-mail <span class="text-red-400">*</span></label>
                <input type="email" name="email" value="{{ old('email') }}" required
                    class="w-full bg-gray-800 border border-white/10 text-white px-4 py-2.5 rounded-xl focus:outline-none focus:border-purple-500 text-sm">
            </div>
            <div class="sm:col-span-2">
                <label class="block text-sm text-gray-400 mb-1.5">Tipo di artista <span class="text-red-400">*</span></label>
                <select name="artist_type" required class="w-full bg-gray-800 border border-white/10 text-gray-300 px-4 py-2.5 rounded-xl focus:outline-none focus:border-purple-500 text-sm">
                    <option value="individual" {{ old('artist_type', 'individual') === 'individual' ? 'selected' : '' }}>Individuale</option>
                    <option value="company" {{ old('artist_type') === 'company' ? 'selected' : '' }}>Azienda</option>
                </select>
            </div>
        </div>
        <div class="flex justify-end space-x-3">
            <a href="{{ route('admin.users') }}" class="px-5 py-2 bg-white/5 hover:bg-white/10 text-gray-300 rounded-xl border border-white/10 transition text-sm">Cancellare</a>
            <button type="submit"
                    :disabled="loading"
